...
public class Board extends GridPane {
	...
	private void fillWithArrows() {
		...
		for (int i = 1 + 1; i < gridPaneSize; i++) {
			this.add(new Arrow(new Double[] { 0.0, 10.0, -10.0, -10.0, 10.0, -10.0 }, 90), gridPaneSize + 1, i);
		}
		for (int i = 1 + 1; i < gridPaneSize; i++) {
			this.add(new Arrow(new Double[] { 0.0, 10.0, -10.0, -10.0, 10.0, -10.0 }, -90), 0, i);
		}
		for (int i = 1 + 1; i < gridPaneSize; i++) {
			this.add(new Arrow(new Double[] { 0.0, 10.0, -10.0, -10.0, 10.0, -10.0 }, 0), i, 0);
		}
		for (int i = 1 + 1; i < gridPaneSize; i++) {
			this.add(new Arrow(new Double[] { 0.0, 10.0, -10.0, -10.0, 10.0, -10.0 }, 180), i, gridPaneSize + 1);
		}
	...
	public Arrow getArrowByCoords(int col, int row) {
		Arrow arrow = (Arrow) this.getChildren().stream()
		...
		this.addEventFilter(MouseEvent.MOUSE_CLICKED, (event) -> {
			...
			for (Node node : this.getChildren()) {
				if (node instanceof Arrow) {
					if (((Arrow) node).isClickable(this) && GridPane.getColumnIndex(node) == clickColNum
							&& GridPane.getRowIndex(node) == clickRowNum) {
